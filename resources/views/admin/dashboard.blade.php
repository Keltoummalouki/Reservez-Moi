...
<head>
    ...
    <meta name="description" content="Tableau de bord administrateur - Gérer les Service Providers sur Reservez-Moi.">
    <meta name="keywords" content="admin, service providers, gestion">
...
<body class="bg-gray-50 font-sans antialiased overflow-x-hidden">
    ...
    <section class="min-h-screen pt-24 pb-10 bg-gradient-to-br from-indigo-100 to-gray-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 class="text-4xl font-bold text-center text-gray-900 mb-8">Tableau de bord Administrateur</h2>
            <div class="flex justify-between items-center mb-6">
                <h3 class="text-2xl font-semibold text-gray-800">Gérer les Service Providers</h3>
                <a href="{{ route('admin.service_providers.create') }}" class="bg-indigo-700 text-white px-6 py-3 rounded-full font-medium hover:bg-indigo-800 transition duration-300 shadow-md hover:shadow-lg">
                    Ajouter un Service Provider
                </a>
            </div>
            ...
            <!-- Liste des Service Providers -->
            <div class="space-y-6">
                @forelse ($serviceProviders as $provider)
                    <div class="bg-white p-6 rounded-2xl shadow-lg">
                        <div class="flex items-center mb-4">
                            <i class="fas fa-user-tie text-indigo-700 text-3xl mr-3"></i>
                            <h3 class="text-xl font-semibold text-gray-900">{{ $provider->name }}</h3>
                        </div>
                        <p class="text-gray-600 mb-2"><strong>Email :</strong> {{ $provider->email }}</p>
                        <p class="text-gray-600 mb-4"><strong>Nombre de services :</strong> {{ $provider->services->count() }}</p>

                        <!-- Liste des services -->
                        @if ($provider->services->isNotEmpty())
                            <div class="mb-4">
                                <h4 class="text-lg font-medium text-gray-800 mb-2">Services offerts :</h4>
                        ...
                        @else
                            <p class="text-gray-600 mb-4">Aucun service offert pour le moment.</p>
                        @endif

                        <!-- Actions -->
                        <div class="flex space-x-3">
                            <a href="{{ route('admin.service_providers.edit', $provider) }}" class="bg-indigo-700 text-white px-4 py-2 rounded-full font-medium hover:bg-indigo-800 transition duration-300 shadow-md hover:shadow-lg">
                                Modifier
                            </a>
                            <form action="{{ route('admin.service_providers.destroy', $provider) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce Service Provider ? Cela supprimera également tous ses services.');">
                                @csrf
                                @method('DELETE')
                ...
                @empty
                    <div class="text-center text-gray-600">
                        <p>Aucun Service Provider trouvé.</p>
                    </div>
                @endforelse
            </div>
        </div>
