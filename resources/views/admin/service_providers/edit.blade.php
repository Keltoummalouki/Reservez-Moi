...
<head>
    ...
    <meta name="description" content="Modifier un Service Provider sur Reservez-Moi.">
    <meta name="keywords" content="admin, service provider, modifier">
    <title>Modifier Service Provider - Reservez-Moi</title>
...
<body class="bg-gray-50 font-sans antialiased overflow-x-hidden">
    ...
    <section class="min-h-screen pt-24 pb-10 bg-gradient-to-br from-indigo-100 to-gray-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 class="text-4xl font-bold text-center text-gray-900 mb-8">Modifier Service Provider</h2>
            ...
            <div class="bg-white p-6 rounded-2xl shadow-lg max-w-lg mx-auto">
                <form action="{{ route('admin.service_providers.update', $user) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-4">
                        <label for="name" class="block text-gray-700 font-medium mb-2">Nom</label>
                        <input type="text" name="name" id="name" value="{{ old('name', $user->name) }}" class="w-full px-4 py-3 rounded-full border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-transparent shadow-sm text-gray-900 transition duration-300" required>
                        @error('name')
                    ...
                    <div class="mb-4">
                        ...
                        <input type="email" name="email" id="email" value="{{ old('email', $user->email) }}" class="w-full px-4 py-3 rounded-full border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-transparent shadow-sm text-gray-900 transition duration-300" required>
                        @error('email')
                            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
