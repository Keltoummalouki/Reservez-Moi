...
<head>
    ...
    <meta name="description" content="Ajoutez un nouveau service sur Reservez-Moi.">
    <meta name="keywords" content="service provider, ajouter service, réservations">
    <title>Ajouter un Service - Reservez-Moi</title>
...
<body class="bg-gray-50 font-sans antialiased overflow-x-hidden">
    ...
    <section class="min-h-screen flex items-center justify-center pt-20 pb-10 bg-gradient-to-br from-indigo-100 to-gray-50">
        <div class="max-w-md w-full bg-white p-8 rounded-3xl shadow-lg transform transition-all duration-500 animate-slide-in-up">
            <h2 class="text-3xl font-bold text-center text-gray-900">Ajouter un Service</h2>
            <p class="mt-2 text-center text-gray-600">Créez un nouveau service pour vos clients</p>
            ...
            <!-- Create Service Form -->
            <form class="space-y-6" action="{{ route('provider.services.store') }}" method="POST">
                @csrf
                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700">Nom du Service</label>
                    <div class="mt-1 relative">
                        <input id="name" name="name" type="text" required value="{{ old('name') }}" class="w-full px-4 py-3 rounded-full border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-transparent shadow-sm text-gray-900 transition duration-300" placeholder="Ex. Coiffure">
                ...
                <div>
                    ...
                    <div class="mt-1 relative">
                        <textarea id="description" name="description" rows="3" class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-transparent shadow-sm text-gray-900 transition duration-300" placeholder="Décrivez votre service...">{{ old('description') }}</textarea>
                    </div>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <div class="mt-1 relative">
                        <input id="price" name="price" type="number" step="0.01" required value="{{ old('price') }}" class="w-full px-4 py-3 rounded-full border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-transparent shadow-sm text-gray-900 transition duration-300" placeholder="Ex. 30.00">
                ...
                <div>
                    ...
                    <div class="mt-1 relative">
                        <input id="category" name="category" type="text" value="{{ old('category') }}" class="w-full px-4 py-3 rounded-full border border-gray-300 focus:ring-2 focus:ring-indigo-500 focus:border-transparent shadow-sm text-gray-900 transition duration-300" placeholder="Ex. Beauté">
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <input id="is_available" name="is_available" type="checkbox" value="1" {{ old('is_available', 1) ? 'checked' : '' }} class="h-5 w-5 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500">
                    </div>
                    @error('is_available')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                <button type="submit" class="w-full bg-indigo-700 text-white py-3 rounded-full font-medium hover:bg-indigo-800 transition duration-300 shadow-md hover:shadow-lg transform hover:-translate-y-1">Ajouter le Service</button>
            </form>
        </div>
    </section>
